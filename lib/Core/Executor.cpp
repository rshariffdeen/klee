  ...
  if (dyn_cast<ReadExpr>(expr)) {
    ...
    if (head) {
      for (const UpdateNode *un = head; un; un = un->next) {
        ...
        //                    PC << "U" << updateCounter << ":";
        //                    updateBindings.insert(std::make_pair(un,
        //                    updateCounter++)); openedList = nextShouldBreak =
        //                    false;
        //                }
...
void Executor::callExternalFunction(ExecutionState &state, KInstruction *target,
  ...
  if (usingSeeds) {
    //        errs() << "\n concretizing function arguments \n";
    for (std::vector<ref<Expr>>::iterator ai = arguments.begin(),
                                          ae = arguments.end();
         ai != ae; ++ai) {
  ...
  for (std::vector<ref<Expr>>::iterator ai = arguments.begin(),
    ...
    if (ExternalCalls ==
      ...
      *ai = optimizer.optimizeExpr(*ai, true);
      ref<ConstantExpr> ce;
      //            errs() << "\n[callExternalFunction]3523\n";
      bool success = solver->getValue(state, *ai, ce);
      //            errs() << "\n[callExternalFunction]3525\n";
      assert(success && "FIXME: Unhandled solver failure");
      (void)success;

      ce->toMemory(&args[wordIndex]);
      //            errs() << "\n[callExternalFunction]3529\n";
      ObjectPair op;
      // Checking to see if the argument is a pointer to something
      if (ce->getWidth() == Context::get().getPointerWidth() &&
          state.addressSpace.resolveOne(ce, op)) {
        //                errs() << "\n[callExternalFunction]3533\n";
        //                op.second->flushToConcreteStore(solver, state);
        //                errs() << "\n[callExternalFunction]3535\n";
      }
...
bool Executor::getSymbolicSolution(
  ...
  for (unsigned i = 0; i != state.symbolics.
